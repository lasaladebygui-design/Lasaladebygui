...
**🎬 Mis imprescindibles y ✨ Sugeridas** (`apps.accounts.models.FavoriteMovie`): dos botones en el propio perfil, cada uno lleva a su **página propia** (no pestañas en la misma pantalla) — `/cuenta/favoritas/essential/` y `/cuenta/favoritas/suggested/`. Ahí dentro, un único buscador con el mismo selector **Películas / Series / Ambas** que el catálogo — no dos cajas de búsqueda separadas, todo en una (el filtro de tipo es cliente, con Alpine, no recarga la página). **Sin límite de cuántas puedes añadir** en ninguno de los dos apartados. Reutiliza el catálogo de `apps.movies` (misma búsqueda en vivo contra TMDb que la tier list de Top Secret: escribes, salen resultados de los dos tipos a la vez, le das a "Añadir"). Cada tarjeta tiene botones ◀ ✕ ▶ para reordenarla dentro de su grupo (película/serie) y, solo en **Sugeridas**, pulsar el propio póster abre un cuadro para explicar **por qué la recomiendas** (`FavoriteMovie.note`, hasta 280 caracteres, con un 💬 de aviso si ya tiene una) — no tiene sentido en Imprescindibles, así que no aparece ahí. Solo puedes editar las tuyas; en el perfil público de otro usuario (`/social/usuarios/<username>/favoritas/<categoría>/`) se ven en modo solo lectura, notas incluidas.
...
- **Guardadas:** lo que has guardado desde la ficha de cualquier película (botón "Guardar película" / "✓ Guardada", `apps.movies.models.SavedMovie` — una fila única por usuario+película, pensado para marcar "quiero verla" sin necesidad de puntuarla todavía). Esta es la lista de la que tira el Modo 2 de la ruleta, de ahí que tenga su propio apartado en vez de ser una sección más dentro de "Mis películas". Tiene botones ▲/▼ para reordenarlas según su orden de importancia (`SavedMovie.order`).
  - **Sublistas** (`apps.movies.models.SavedMovieList`, ej. "Terror", "Para ver en familia"): creas las tuyas con "+ Nueva sublista" y asignas cada guardada a una desde un desplegable en su fila. Un selector arriba filtra Guardadas por sublista (o "Todas"), y **la ruleta Modo 2 admite el mismo filtro** (`?list=<id>`) para girar solo entre las de esa sublista en vez de sobre todas las guardadas a la vez. Borrar una sublista no borra las películas, solo las deja sin sublista.
...
A diferencia del botón .ics (descarga manual, una vez, sin seguir sincronizado), esto es una conexión de verdad vía **OAuth 2.0**: conectas tu cuenta de Google una vez y, a partir de ahí, cada evento que añadas a tu calendario personal de Top Secret (sección 8) se crea solo en tu Google Calendar — sin volver a tocar nada. El botón de conectar/desconectar vive en la propia página del calendario (de forma minimalista, una línea de texto arriba del mes), no en el perfil.
