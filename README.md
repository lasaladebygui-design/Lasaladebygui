...
**🎬 Mis imprescindibles y ✨ Sugeridas** (`apps.accounts.models.FavoriteMovie`): dos pestañas paralelas (Alpine.js), y dentro de cada una un único buscador con el mismo selector **Películas / Series / Ambas** que el catálogo — no dos cajas de búsqueda separadas, todo en una (el filtro de tipo es cliente, con Alpine, no recarga la página). Imprescindibles admite 3 películas + 3 series, Sugeridas admite 6 películas + 3 series (`FavoriteMovie.LIMITS`, por `(categoría, media_type)`). Reutiliza el catálogo de `apps.movies` (misma búsqueda en vivo contra TMDb que la tier list de Top Secret: escribes, salen resultados de los dos tipos a la vez, le das a "Añadir"). Cada tarjeta tiene botones ◀ ✕ ▶ para reordenarla dentro de su grupo (película/serie) y, solo en **Sugeridas**, un desplegable opcional para explicar **por qué la recomiendas** (`FavoriteMovie.note`, hasta 280 caracteres) — no tiene sentido en Imprescindibles, así que no aparece ahí. Solo puedes editar las tuyas; en el perfil público de otro usuario (`/social/usuarios/<username>/`) se ven en modo solo lectura, notas incluidas. Al llegar al límite de un grupo, "Añadir" deja de funcionar (con un aviso) hasta que quites alguna.
...
- **d) Otros** (`/top-secret/dentro/otros/`): agrupa la Tier list, el Tablón de fotos y el Calendario, apartados que no son "la lista numerada de Quentin" sino cosas aparte:
  ...
  - **Calendario** (`/top-secret/dentro/calendario/`, `apps.movies.models.ReleaseEvent`): qué película o serie toca cada fecha — **privado, dentro del maletín**, no en el catálogo público. Se añade pulsando el número de un día del mes (revela una búsqueda en vivo ahí mismo, movie+series a la vez) en vez de un formulario fijo aparte. Cada evento muestra su portada y su título en la propia celda del día. Al añadirlo se manda una notificación push (sección 11) a los usuarios suscritos y, para quien tenga **Google Calendar conectado de verdad** (ver sección 11), se crea solo en su calendario. Cada evento tiene además un botón para descargar su `.ics` a mano (Apple Calendar, Outlook, o Google Calendar sin conectar).
...
A diferencia del botón .ics (descarga manual, una vez, sin seguir sincronizado), esto es una conexión de verdad vía **OAuth 2.0**: cada usuario conecta su cuenta de Google una vez desde su perfil y, a partir de ahí, cada evento que se añada al calendario de Top Secret (sección 8) se crea solo en su Google Calendar — sin volver a tocar nada.
...
- **Al añadir/quitar un evento:** igual que el aviso push, es una difusión a **todos** los usuarios conectados (no solo a quien lo añadió) — `apps.secret.views._sync_event_to_connected_calendars` crea el evento en el Google Calendar de cada uno y guarda el id que Google le asigna en `apps.movies.models.ReleaseEventGoogleLink` (uno por evento y usuario, porque cada quien tiene su propia copia); al quitar el evento del sitio, se borra también de cada calendario conectado usando esos ids.
- **Desconectar** (`accounts:google-calendar-disconnect`): borra la conexión y los enlaces de eventos que ya no sirven para nada (sin conexión no hay con qué borrarlos de Google si algún día se quitan del sitio) — las copias ya creadas en el Google Calendar del usuario no se tocan, se quedan ahí como cualquier evento normal.
