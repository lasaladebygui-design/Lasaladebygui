...
- **d) Otros** (`/top-secret/dentro/otros/`): agrupa la Tier list, el Tablón de fotos y el Calendario, apartados que no son "la lista numerada de Quentin" sino cosas aparte:
  ...
  - **Calendario** (`/top-secret/dentro/calendario/`, `apps.movies.models.ReleaseEvent`): qué película o serie toca cada fecha — **privado, dentro del maletín**, no en el catálogo público. Busca un título (combina resultados de película y de serie a la vez) y elige la fecha para añadirlo, mismo patrón de búsqueda en vivo que la tier list. Al añadir un evento se manda una notificación push (sección 11) a los usuarios suscritos. No hay sincronización real con Google Calendar (exigiría OAuth y credenciales por usuario); en su lugar, cada evento tiene un botón para descargar un `.ics` (generado a mano, sin librerías externas) que se importa con un clic en Google Calendar, Apple Calendar, Outlook...
