...
- **c) Lista completa:** todas las entradas, con su nota, géneros y comentario. **Ya no es una única lista de Bygui:** cualquiera con cuenta tiene la suya propia (mismo selector numérico, buscador por nota y lista completa, pero con sus propias entradas, listas y numeración), la gestiona entera desde la web (añadir buscando en TMDb, puntuar, comentar, crear listas — sin pasar por el admin) y decide con quién la comparte, de solo lectura (ver "Compartidos" más abajo). La de Bygui (`owner=None` en `apps.secret.models.SecretMovie`/`Genre`) sigue siendo la original, gestionada desde el admin y editable desde la web solo por ella misma (con **TopSecretConfig.allow_web_editing** activo, igual que siempre); es la que ve cualquiera con el código por defecto si no tiene cuenta, y a la que cualquier usuario con cuenta puede echar un vistazo (de solo lectura) desde el selector "🕶️ La de Bygui". Qué lista se está viendo/editando viaja en la URL como `?scope=own|bygui|<username>` (ver `apps.secret.views._resolve_scope`).
- **d) Otros** (`/top-secret/dentro/otros/`): agrupa la Tier list, el Tablón de fotos, el Calendario y Compartidos, apartados que no son "la lista numerada" sino cosas aparte:
  - **Tier list:** un ranking de películas por niveles (`apps.secret.models.TierLevel` + `TierListEntry`) — esta sigue siendo única y global (la de Bygui), no personal como la lista completa. Los niveles **no son fijos**: nombre, color y orden se gestionan enteros desde la propia página del Tier List (un ✏️ junto a cada nivel abre un mini formulario de nombre+color; "+ Añadir nivel" crea uno nuevo al final; 🗑️ lo borra) — no hace falta tocar el admin para nada de esto, aunque también aparecen ahí (**Admin → Top Secret → Niveles de tier list**) por si se prefiere. Al borrar un nivel, sus películas no se pierden: vuelven a "Sin clasificar" (`on_delete=SET_NULL`). Añadir películas (buscador TMDb en vivo) y arrastrar y soltar (`static/js/tier_list.js`, drag-and-drop nativo del navegador, sin librerías) funciona igual que antes: cada entrada se coloca al final del nivel de destino, y las recién añadidas caen en **"Sin clasificar"** (fila con borde discontinuo, encima de los niveles reales) hasta que se arrastran a uno. Un botón **"Vaciar tier list"** (con confirmación) borra todas las entradas — no los niveles — para volver a empezar de cero.
  - **Tablón de fotos** (`/top-secret/dentro/tablon/`): a diferencia del resto de Top Secret, **esto sí se sube desde la propia web, no desde el admin** — cualquiera que haya entrado con el código puede subir una foto con una pequeña descripción (`apps.secret.models.SecretPhoto`). Si has iniciado sesión, puedes elegir con una casilla **"Publicar como anónimo"** si quieres que se guarde tu nombre (coloreado por rango) o no; sin cuenta, siempre se sube como "Anónimo" (no hay identidad que ocultar ni mostrar). Con estética de cine propia (carrete de 35mm: fondo oscuro, perforaciones arriba y abajo de cada foto, texto a máquina de escribir) en vez del tema visual activo del sitio, deliberadamente distinta para que se note que es "el cuarto oscuro" del maletín. Cada usuario tiene el suyo, privado por defecto — decide con quién lo comparte (`apps.secret.models.PhotoBoardMember`).
  - **Calendario** (`/top-secret/dentro/calendario/`, `apps.movies.models.ReleaseEvent`): qué película o serie toca cada fecha — personal de cada usuario, hace falta el código del maletín **y** haber iniciado sesión. Privado por defecto, pero ya no es imposible compartirlo: decides con quién (`apps.secret.models.CalendarShareMember`) desde "🔗 Compartir", de solo lectura para quien invites. Se añade pulsando el número de un día del mes (revela una búsqueda en vivo ahí mismo, movie+series a la vez) en vez de un formulario fijo aparte; el 💬 del día deja además un comentario suelto sin ligarlo a ninguna película (`apps.movies.models.CalendarDayNote`). Cada evento muestra su portada y su título en la propia celda del día. El botón para conectar/desconectar **Google Calendar de verdad** (ver sección 11) vive aquí mismo, minimalista, arriba del mes, y solo para el dueño — si lo tienes conectado, cada evento se crea solo en tu calendario (`ReleaseEvent.google_event_id`); si no, hay un botón para descargar su `.ics` a mano.
  - **Compartidos** (`/top-secret/dentro/compartidos/`): un único sitio para ver y gestionar, con quién compartes tu lista propia, tu tablón de fotos y tu calendario — antes había que entrar a cada apartado por separado para gestionar su propio "compartir"; los interruptores de aquí reutilizan las mismas invitar/expulsar de cada apartado (`SecretListMember`, `PhotoBoardMember`, `CalendarShareMember`), así que activarlos ahí o desde aquí es lo mismo. También lista qué te han compartido a ti, con acceso directo a la lista/tablón/calendario de cada amigo.
